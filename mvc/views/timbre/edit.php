{{ include('layouts/header.php', {title:'Changer le Timbre'})}}

<main>
    <div class="container flex-col-center">
        <h1 class="milieu">Changer le Timbre</h1>
        <form method="post" class="flex-col-center" enctype="multipart/form-data">
            ...
            <label class="image">
                Image Principale (ajouter un fichier si vous voulez changer l'image)
                {% for image in images %}
                    {% if image.ordre_daffichage == 0 %}
                        <img src="{{ asset ~ image.image_url }}" alt="{{ image.description_courte }}">
                    {% endif %}
                {% endfor %}
                <input type="file" name="image_un" accept="image/jpeg,image/png,image/webp" data-max-size="5242880">
            </label>
            ...
            <label class="image">
                Image Secondaire (ajouter un fichier si vous voulez changer l'image)
                {% for image in images %}
                    {% if image.ordre_daffichage == 1 %}
                        <img src="{{ asset ~ image.image_url }}" alt="{{ image.description_courte }}">
                    {% endif %}
                {% endfor %}
            ...
            <label class="image">
                Image Trois (ajouter un fichier si vous voulez changer l'image)
                {% for image in images %}
                    {% if image.ordre_daffichage == 2 %}
                        <img src="{{ asset ~ image.image_url }}" alt="{{ image.description_courte }}">
                    {% endif %}
                {% endfor %}
            ...
            <label class="image">
                Image Quatre (ajouter un fichier si vous voulez changer l'image)
                {% for image in images %}
                    {% if image.ordre_daffichage == 3 %}
                        <img src="{{ asset ~ image.image_url }}" alt="{{ image.description_courte }}">
                    {% endif %}
                {% endfor %}
            ...
            {% endif %}
            <input class="bouton bouton-secondaire" type="submit" value="Soumettre">
        </form>
    </div>
</main>
